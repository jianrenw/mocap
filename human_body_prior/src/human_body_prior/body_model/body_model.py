...
class BodyModel(nn.Module):
    ...
    def __init__(
        ...
        self.use_expression = (
            self.model_type in ["smplx", "flame"] and num_expressions is not None
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if pose_hand is None and self.model_type not in [
            "animal_horse",
            "animal_dog",
            "animal_rat",
            "flame",
        ...
        if self.model_type in ["smplh", "smpl"]:
            ...
            full_pose = torch.cat(
                [root_orient, pose_body, pose_jaw, pose_eye], dim=-1
            )  # orient:3, body:63, jaw:3, eyel:3, eyer:3, handl, handr
        elif self.model_type in [
            "mano",
        ]:
