...
from human_body_prior.body_model.lbs import batch_rodrigues, lbs
...
class RigidObjectModel(nn.Module):
    ...
    def __init__(self, plpath, batch_size=1, dtype=torch.float32):
        ...
        self.register_parameter(
            "root_orient", nn.Parameter(root_orient, requires_grad=True)
        )

        mesh = Mesh(filename=plpath)

        self.rigid_v = torch.from_numpy(
            np.repeat(mesh.v[np.newaxis], batch_size, axis=0)
        ).type(dtype)
    ...
    def forward(self, root_orient, trans):
        ...
        if trans is None:
            trans = self.trans
        verts = torch.bmm(self.rigid_v, batch_rodrigues(root_orient)) + trans.view(
            -1, 1, 3
        )
