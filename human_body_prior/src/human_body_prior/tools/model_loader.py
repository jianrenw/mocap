...
def exprdir2model(expr_dir, model_cfg_override: dict = None):
    ...
    available_ckpts = sorted(
        glob.glob(osp.join(model_snapshots_dir, "*.ckpt")), key=osp.getmtime
    )
    assert len(available_ckpts) > 0, ValueError(
        "No checkpoint found at {}".format(model_snapshots_dir)
    )
    trained_weights_fname = available_ckpts[-1]

    model_cfg_fname = glob.glob(
        osp.join("/", "/".join(trained_weights_fname.split("/")[:-2]), "*.yaml")
    )
...
def load_model(
    ...
    model_cfg, trained_weights_fname = exprdir2model(
        expr_dir, model_cfg_override=model_cfg_override
    )
    ...
    if (
        disable_grad
    ):  # i had to do this. torch.no_grad() couldnt achieve what i was looking for
    ...
    if comp_device == "cpu" or not torch.cuda.is_available():
        ...
        state_dict = torch.load(
            trained_weights_fname, map_location=torch.device("cpu")
        )["state_dict"]
    else:
    ...
    if remove_words_in_model_weights is not None:
        ...
        state_dict = {
            k.replace(words, "") if k.startswith(words) else k: v
    ...
    model_instance.eval()
    logger.info(
        f"Loaded model in eval mode with trained weights: {trained_weights_fname}"
    )
