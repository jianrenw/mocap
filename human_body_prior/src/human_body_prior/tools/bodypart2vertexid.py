...
def find_handVertexIDs(blend_weights, all_partIds, interested_partIds):
    ...
    print(
        "isSorted =",
        all(VertexIDs[i] <= VertexIDs[i + 1] for i in range(len(VertexIDs) - 1)),
    )
...
def smplx_part_ids():
    ...
    bm = BodyModel(
        bm_fname="/ps/project/common/moshpp/smplx/locked_head/model_6_merged_exp_hands_fixed_eyes/neutral/model.npz"
    )
    ...
    joints = np.load(
        "/ps/project/supercap/support_files/smplx/smplx_downsampled/328/female/model.npz"
    )["joints"]
    ...
    meshes = [
        Mesh(v=body_v[part2vids[partname]], f=[], vc=part_vc)
        for partname, part_vc in body_part_vc.items()
    ]
...
def smplh_part_ids():
    ...
    bm = BodyModel(
        bm_fname="/ps/scratch/common/moshpp/smplh/locked_head/female/model.npz"
    )
