...
def aa2matrot(pose):
    ...
    pose_body_matrot = tgm.angle_axis_to_rotation_matrix(pose)[
        :, :3, :3
...
def tmat(R, t):
    ...
    return torch.cat(
        [
            F.pad(R.view(-1, 3, 3), [0, 0, 0, 1]),
            F.pad(t.view(-1, 3, 1), [0, 0, 0, 1], value=1),
        ],
        dim=2,
    ).view(bs, -1, 4, 4)
...
def batch_rigid_transform(rot_mats, joints, parents):
    ...
    for i in range(1, parents.shape[0]):
        ...
        curr_res = torch.matmul(
            transform_chain[parents[i]], tmat(rot_mats[:, i], rel_joints[:, i])[:, 0]
        )
