...
def evaluate_model(
    dataset_dir, vp_model, vp_ps, batch_size=5, save_upto_bnum=10, splitname="test"
):
    ...
    outpath = os.path.join(
        ...
        "evaluations",
        "ds_%s" % ds_name,
        os.path.basename(vp_ps.best_model_fname).replace(".pt", ""),
        "%s_samples" % splitname,
    )
    ...
    for bId, dorig in enumerate(ds):
        ...
        imgpath = makepath(
            os.path.join(outpath, "%s-%03d.png" % (vp_ps.expr_code, bId)), isfile=True
        )
        VPoserTrainer.vis_results(
            dorig, bm, vp_model, imgpath, view_angles=[0, 180]
        )  # , view_angles = [0, 180, 90])
...
def evaluate_error(dataset_dir, vp_model, vp_ps, batch_size=512):
    ...
    for splitname in ["test", "train", "vald"]:
        ...
        ds = DataLoader(
            ds, batch_size=batch_size, shuffle=False, drop_last=True
        )  # batchsize for bm is fixed so drop the last one
        ...
        with torch.no_grad():
            for dorig in tqdm(ds):
                ...
                for k in dorig:  # whatever field is missing from drec copy from dorig
                    if k not in drec:
                        ...
                        if len(final_errors) == 0 and len(loss_mean) == 0:
                            print(
                                "Field %s is not predicted by the model and is copied from the original data."
                                % k
                            )
                ...
                loss_mean.append(
                    torch.mean(torch.abs(body_orig - body_rec) * MESH_SCALER)
                )
    ...
    outpath = makepath(
        os.path.join(
            ...
            "evaluations",
            "ds_%s" % ds_name,
            os.path.basename(vp_ps.best_model_fname).replace(".pt", ".json"),
        ),
        isfile=True,
    )
...
if __name__ == "__main__":
    ...
    print(
        "[%s] [DS: %s] -- %s"
        % (
            vp_ps.best_model_fname,
            dataset_dir,
            ", ".join(
                ["%s: %.2e" % (k, v["v2v_mae"]) for k, v in final_errors.items()]
            ),
        )
    )
