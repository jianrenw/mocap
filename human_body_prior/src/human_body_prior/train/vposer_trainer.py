...
from human_body_prior.tools.omni_tools import (get_support_data_dir, log2file,
                                               make_deterministic, makepath)
...
class VPoserTrainer(LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        metrics = {
            "val_loss": np.nanmean(np.concatenate([v["val_loss"] for v in outputs]))
        }
        ...
        if self.global_rank == 0:
            ...
            self.text_logger(
                "Epoch {}: {}".format(
                    self.current_epoch,
                    ", ".join("{}:{:.2f}".format(k, v) for k, v in metrics.items()),
                )
            )
            self.text_logger(
                "lr is {}".format(
