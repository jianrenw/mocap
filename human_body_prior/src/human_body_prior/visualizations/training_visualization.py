        ...
        for rId in range(nc):
            for cId in range(nc):
                ...
                out_image[
                    ...
                    0,
                    0,
                    (rId * width) : ((rId + 1) * width),
                    (cId * height) : ((cId + 1) * height),
                ] = cv2.cvtColor(img_resized, cv2.COLOR_BGRA2RGBA)
...
def vposer_trainer_renderer(bm, num_bodies_to_display=5):
    ...
    def render_once(
        body_parms, body_colors=[colors["grey"], colors["brown-light"]], out_fname=None
    ):
        """
        ...
        assert len(body_parms) <= len(body_colors), ValueError(
            "Not enough colors provided for #{} body_parms".format(len(body_parms))
        )
        ...
        body_evals = [
            c2c(
                bm(
                    root_orient=(
                        v["root_orient"].view(bs, -1)
                        if "root_orient" in v
                        else torch.zeros(bs, 3).type_as(v["pose_body"])
                    ),
                    pose_body=v["pose_body"].contiguous().view(bs, -1),
                ).v
            )
            for v in body_parms
        ]
        ...
        for bId in body_ids:
            ...
            for body, body_color in zip(body_evals, body_colors):
                cur_body_mesh = Mesh(
                    body[bId], faces, vertex_colors=np.ones([num_verts, 3]) * body_color
                )
                concat_cur_meshes = (
                    cur_body_mesh
                    if concat_cur_meshes is None
                    else mesh_cat(concat_cur_meshes, cur_body_mesh)
                )
