...
def set_t_pose(robot, t_pose_angles):
    ...
    joint_position_right_elbow, _ = p.getLinkState(robot, joint_names.index('elbow_Right'))[4:6]
    joint_dis = np.linalg.norm(np.array(joint_position_right_shoulder) - np.array(joint_position_right_elbow))
    print('arm', joint_dis)

    joint_position_right_hip, _ = p.getLinkState(robot, joint_names.index('hipRoll_Right'))[4:6]
    joint_position_right_knee, _ = p.getLinkState(robot, joint_names.index('kneePitch_Right'))[4:6]
    joint_dis = np.linalg.norm(np.array(joint_position_right_hip) - np.array(joint_position_right_knee))
    print('leg', joint_dis)
