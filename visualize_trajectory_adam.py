...
actor_dof_name = gym.get_actor_dof_names(env, actor_handle)
print(actor_dof_name)
...
def draw_reference(gym, viewer, env_handle, body_pos, radius=0.01):
    ...
    for ubp in updated_body_pos:
        ...
        sphere_geom = gymutil.WireframeSphereGeometry(
            radius, 10, 10, None, color=(1, 0, 0)
        )
        gymutil.draw_lines(sphere_geom, gym, viewer, env_handle, sphere_pose)


def draw_contact(
    gym, viewer, env_handle, body_pos, left_foot_contact, right_foot_contact
):
    if left_foot_contact:
        left_foot_pos = body_pos[rigid_body_names.index("toeLeft")]
        sphere_pose = gymapi.Transform(
            gymapi.Vec3(left_foot_pos[0], left_foot_pos[1], left_foot_pos[2]), r=None
        )
        sphere_geom = gymutil.WireframeSphereGeometry(
    ...
    if right_foot_contact:
        ...
        sphere_pose = gymapi.Transform(
            gymapi.Vec3(right_foot_pos[0], right_foot_pos[1], right_foot_pos[2]), r=None
        )
        sphere_geom = gymutil.WireframeSphereGeometry(
            0.05, 10, 10, None, color=(1, 0, 0)
        )
...
while True:
    for i in range(frame_num):
        ...
        draw_contact(
            ...
            viewer,
            env,
            rigid_body_state[:, 0:3],
            left_foot_contact[i],
            right_foot_contact[i],
        )
