...
def skeleton2adam(skeleton):
    ...
    l_elbow_rot_y = l_elbow_rot_y / np.linalg.norm(l_elbow_rot_y, axis=1, keepdims=True)
    ...
    r_elbow_rot_y = r_elbow_rot_y / np.linalg.norm(r_elbow_rot_y, axis=1, keepdims=True)
    ...
    l_foot_angle = (
        np.arccos(cos) - np.arctan(0.047 / 0.160) - np.pi / 2 + 6 * np.pi / 180
    )
...
if __name__ == "__main__":
    ...
    for data_file in data_files:
        ...
        urdf_path = "{}/mocap/robots/adam_standard/urdf/adam_standard_low.urdf".format(
            home_dir
        )
        ...
        joblib.dump(
            result, os.path.join(args.out_dir, data_file.replace(".json", ".pt"))
        )
